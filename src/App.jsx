...
// ─── v2 additions: z-score + caching ─────────────────────────
var CACHE_TTL = 6 * 3600 * 1000; // 6 hours
function cacheGet(key) { try { var r = localStorage.getItem(key); return r ? JSON.parse(r) : null; } catch(e) { return null; } }
function cacheSet(key, data) { try { localStorage.setItem(key, JSON.stringify({ ts: Date.now(), data: data })); } catch(e) {} }
function cacheIsStale(key) { var c = cacheGet(key); if (!c || !c.ts) return true; return Date.now() - c.ts > CACHE_TTL; }

function clip(v) { return Math.max(-3, Math.min(3, v)); }
function zScoreSingle(value, arr) {
  if (!arr || arr.length < 10) return null;
  var mean = 0; for (var i = 0; i < arr.length; i++) mean += arr[i]; mean /= arr.length;
  var variance = 0; for (var i = 0; i < arr.length; i++) variance += (arr[i] - mean) * (arr[i] - mean); 
  var std = Math.sqrt(variance / arr.length) || 1;
  return clip((value - mean) / std);
}

// Enhanced scoreLinear: if historical z-score available, blend it in (70% z, 30% heuristic)
function scoreWithHistory(value, cold, neut, hot, ext, histArr) {
  var heuristicScore = scoreLinear(value, cold, neut, hot, ext);
  if (!histArr || histArr.length < 30) return heuristicScore;
  var z = zScoreSingle(value, histArr);
  if (z === null) return heuristicScore;
  return clip(z * 0.7 + heuristicScore * 0.3);
}
...
export default function App() {
  ...
  var fetchDune = useCallback(function() {
    ...
    var cached = cacheGet("LHI_DUNE");
    if (cached && cached.data && !duneData) setDuneData(cached.data);
    // Only fetch if stale
    if (!cacheIsStale("LHI_DUNE") && cached && cached.data) { return; }
    setDuneLoading(true);
    fetch("/api/dune").then(function(r){return r.json();}).then(function(data){
      if(data.rows){var byDate={};for(var i=0;i<data.rows.length;i++){var row=data.rows[i],dt=row.dt.split(" ")[0];if(!byDate[dt])byDate[dt]={dt:row.dt,borrow:0,repay:0,count:0};byDate[dt].borrow+=row.daily_borrow_usd||0;byDate[dt].repay+=row.daily_repay_usd||0;byDate[dt].count+=row.borrow_count||0;}
        var arr=Object.values(byDate);arr.sort(function(a,b){return a.dt<b.dt?-1:1;});setDuneData(arr);cacheSet("LHI_DUNE", arr);}
      setDuneLoading(false);
    }).catch(function(){setDuneLoading(false);});
  },[]);
  var fetchHistory = useCallback(function() {
    // Load from cache immediately
    var cached = cacheGet("LHI_HIST");
    if (cached && cached.data && !historyData) setHistoryData(cached.data);
    // Only fetch if stale
    if (!cacheIsStale("LHI_HIST") && cached && cached.data) { return; }
    fetch("/api/history").then(function(r){return r.json();}).then(function(data){
      if(data.history && data.history.length > 0) { setHistoryData(data.history); cacheSet("LHI_HIST", data.history); }
    }).catch(function(){});
  },[]);
  ...
  var m = useMemo(function(){
    ...
    // Extract historical arrays for z-score blending
    var hRates = historyData ? historyData.filter(function(d){return d.borrowRate>0;}).map(function(d){return d.borrowRate;}) : null;
    var hUtils = historyData ? historyData.filter(function(d){return d.utilization>0;}).map(function(d){return d.utilization;}) : null;
    var hBorrows = historyData ? historyData.filter(function(d){return d.totalBorrow>0;}).map(function(d){return d.totalBorrow;}) : null;
    var hBmcap = historyData ? historyData.filter(function(d){return d.borrowMcapRatio!=null&&d.borrowMcapRatio>0;}).map(function(d){return d.borrowMcapRatio;}) : null;
    var brS=scoreWithHistory(wBR,3,7,20,45,hRates);var btcF=null,ethF=null;
    ...
    var uS=scoreWithHistory(wU,55,72,87,95,hUtils),tB=totalW;
    var bMR=btcMcap?(tB/btcMcap)*100:null,bMS=bMR!=null?scoreWithHistory(bMR,0.2,0.5,1.2,2.5,hBmcap):0;
    var bAS=scoreWithHistory(tB/1e9,2,6,15,30,hBorrows?hBorrows.map(function(v){return v/1e9;}):null);var volMomS=duneMetrics?scoreLinear(duneMetrics.volChange,-30,0,30,80):0;
  ...
  },[pools,btcMcap,hlData,duneMetrics,historyData]);
  ...
  return (
    <div style={{"--f":f,fontFamily:f,background:"#fafafa",minHeight:"100vh",padding:"16px 14px",color:"#333",fontSize:14}}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
        <div>
          <div style={{fontSize:18,fontWeight:700,color:"#111",letterSpacing:0.3}}>{t.title} <span style={{fontSize:10,color:"#bbb",fontWeight:400}}>v2</span></div>
          <div style={{fontSize:11,color:"#aaa",marginTop:2}}>{t.subtitle} &middot; {t.sources} &middot; <span style={{color:cacheIsStale("LHI_DUNE")?"#ccc":"#0ea371",fontSize:10}}>{cacheIsStale("LHI_DUNE")?"":"cached"}</span></div>
        </div>
      ...
      {loading?<div style={{textAlign:"center",padding:60,color:"#bbb",fontSize:13}}>{t.loading}</div>:!m?<div style={{textAlign:"center",padding:40,color:"#999",fontSize:13}}>{t.no_data}</div>:(<>
        ...
        <div style={{fontSize:10,color:"#bbb",lineHeight:1.8}}>
          ...
          <br /><b style={{color:"#999"}}>v2:</b> {lang==="ko"?"히스토리컬 데이터 있으면 z-score(70%) + 휴리스틱(30%) 블렌딩. localStorage 캐싱 (6시간 TTL) — API 재호출 최소화.":"When historical data available, scoring blends z-score (70%) with heuristic (30%). localStorage caching (6h TTL) minimizes API calls."}
        </div>
